...
const PermissionsPage: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchPermissions = async () => {
      try {
        ...
        console.error("Lỗi khi tải quyền hạn:", err);
        setError("Lỗi khi tải quyền hạn");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handlePermissionChange = async (
    ...
    try {
      ...
      console.error("Có lỗi khi cập nhật quyền hạn:", err);
  ...
  if (error) {
    return (
      <div className="p-4 bg-danger-50 text-danger-500 rounded-md">{error}</div>
    );
  }
        ...
        <Card className="lg:col-span-2 p-4" radius="sm">
          <CardHeader className="pb-0">
            <h3 className="text-lg font-semibold">
              {selectedRole
                ? `Quyền hạn cho ${selectedRole.name}`
                : "Hãy chọn vai trò"}
            </h3>
          </CardHeader>
